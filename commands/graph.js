...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('graph')
    .setDescription('体重またはBMIの推移を表示（例: /graph type:weight range:1m）')
    .addStringOption(option =>
      option.setName('type')
        .setDescription('weight または bmi')
        .setRequired(true)
    )
    .addStringOption(option =>
      option.setName('range')
        .setDescription('1w, 1m, 1y')
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    const rangeMap = { '1w': 7, '1m': 30, '1y': 365 };
    ...
    const config = {
      ...
      options: {
        scales: {
          ...
          y: { title: { display: true, text: (type === 'weight') ? '体重 (kg)' : 'BMI' } }
        }
      }
    };
